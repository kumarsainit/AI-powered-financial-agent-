...
Scope and threshold were chosen by measurement. Against the 25 solved samples, end-to-end field agreement (150 comparisons) scored: no staleness rule 93, expense-scoped at 1.0 **96**, expense-scoped at 1.25/1.5 93, income-scoped at any threshold 90. Applying the rule to income was measurably harmful and is therefore off by default; dropping a lapsed charge is a factual inference about a stopped subscription, whereas dropping lapsed income would only be a guess. Regression tests cover a lapsed series, a current series, and all three scope settings.

**Corrected in Phase 7 — read this before trusting the numbers above.** The rule as first written measured cadence with a fixed 30-day month, so any monthly series whose last gap ran 31–33 days was wrongly declared lapsed; it discarded 47 legitimate monthly expense series across 47 production requests. The 96-versus-93 sample gain reported above was therefore an artifact of that arithmetic error, not evidence for the rule. Phase 7 replaced the approximation with real calendar-month arithmetic (`projection.elapsed_cycles`). Corrected, the rule discards nothing at all on this dataset and sample agreement returns to 93. The rule was kept — it still prevents a genuinely lapsed series from being resurrected, which is covered by tests — but it should be understood as an inert safety guard here rather than a measured improvement. See `phase7_adversarial_validation.md` §5 and §9.
